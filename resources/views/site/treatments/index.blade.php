            ...
            <header class="card-header noborder">
                ...
                <div class="justify-center flex flex-wrap sm:flex items-center lg:justify-end gap-3">
                    <div class="relative w-full sm:w-auto flex items-center">
                        <form id="searchForm" method="get" action="{{ route('treatments.index') }}">
                ...
                <div class="overflow-x-auto -mx-6">
                    <div class="inline-block min-w-full align-middle">
                        <div class="overflow-hidden ">
                            <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                                ...
                                @forelse ($treatments as $treatment)
                                    <tr>
                                        ...
                                        <td class="table-td">
                                            {{ Str::limit($treatment->description,20) ?? 'N/A' }}
                                        </td>

                                        <td class="table-td">
                                            {{ $treatment->created_at->diffForHumans() ?? 'N/A' }}
                                        </td>
                                        ...
                                        <td class="table-td">
                                            <div class="flex space-x-3 rtl:space-x-reverse">
                                                ...
                                                @can('treatment delete')
                                                    <form id="deleteForm{{ $treatment->id }}" method="POST"
                                                          action="{{ route('treatments.destroy', $treatment) }}">
                                                        @csrf
                                                        @method('DELETE')
                                                        <a class="action-btn cursor-pointer"
                                                           onclick="sweetAlertDelete(event, 'deleteForm{{ $treatment->id }}')"
                                                           type="submit">
                            ...
                            <x-table-footer :per-page-route-name="'treatments.index'" :data="$treatments"/>
                        </div>
                    </div>
                </div>
            </div>
