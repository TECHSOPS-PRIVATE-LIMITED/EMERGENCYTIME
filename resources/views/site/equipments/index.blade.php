            ...
            <header class="card-header noborder">
                ...
                <div class="justify-center flex flex-wrap sm:flex items-center lg:justify-end gap-3">
                    <div class="relative w-full sm:w-auto flex items-center">
                        <form id="searchForm" method="get" action="{{ route('equipments.index') }}">
                ...
                <div class="overflow-x-auto -mx-6">
                    <div class="inline-block min-w-full align-middle">
                        <div class="overflow-hidden ">
                            <table class="min-w-full divide-y divide-slate-100 table-fixed dark:divide-slate-700">
                                ...
                                @forelse ($equipments as $equipment)
                                    <tr>
                                        ...
                                        <td class="table-td">
                                            {{ $equipment->last_maintenance_date ?? 'N/A' }}
                                        </td>

                                        <td class="table-td">
                                            {{ Str::limit($equipment->description,20) ?? 'N/A' }}
                                        </td>

                                        <td class="table-td">
                                            {{ $equipment->created_at->diffForHumans() ?? 'N/A' }}
                                        </td>
                                        ...
                                        <td class="table-td">
                                            <div class="flex space-x-3 rtl:space-x-reverse">
                                                ...
                                                <form id="deleteForm{{ $equipment->id }}" method="POST"
                                                      action="{{ route('equipments.destroy', $equipment) }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <a class="action-btn cursor-pointer"
                                                       onclick="sweetAlertDelete(event, 'deleteForm{{ $equipment->id }}')"
                                                       type="submit">
                            ...
                            <x-table-footer :per-page-route-name="'equipments.index'" :data="$equipments"/>
                        </div>
                    </div>
                </div>
            </div>
